...
class ToolRegistry:
    ...
    def _nmap_top_ports(self, action: Action) -> None:
        result = run_command(["nmap", "-Pn", "--top-ports", "100", "-sV", action.target], timeout=180)
    ...
    def _whatweb(self, action: Action) -> None:
        result = run_command(["whatweb", "--no-errors", action.target], timeout=120)
    ...
    def _httpx_probe(self, action: Action) -> None:
        result = run_command(["httpx", "-silent", "-title", "-tech-detect", "-u", action.target], timeout=120)
